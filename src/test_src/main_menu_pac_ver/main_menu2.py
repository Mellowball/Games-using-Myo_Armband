...
#ThreadProcess 클래스 pacman에서 불러와서 시리얼 통신 진행 => main_menu 파일에서 시리얼 획일화 후 진행
# from pacman2 import ThreadProcess as tp
...
# import pacman2    => pacman.py 파일 참조 안 함
...
def pac_man():
    class GameController(object):
        ...
        def checkEvents(self):
            for event in pygame.event.get():
                if event.type == QUIT or receivenumber == 5:
                    main_menu()
    ...
    class Pacman(Entity):
        ...
        def getValidKey(self):
            ...
            if key_pressed[K_UP] or receivenumber == 3:         # Wave Out
                return UP
            if key_pressed[K_DOWN] or receivenumber == 2:       # Wave In
                return DOWN
            if key_pressed[K_LEFT] or receivenumber == 4:       # FingerSpread
                return LEFT
            if key_pressed[K_RIGHT] or receivenumber == 1:      # FIST
                return RIGHT
            if key_pressed[K_ESCAPE] or receivenumber == 5:     # DoubleTap
                # ser.close()
                main_menu() 
            return STOP  
